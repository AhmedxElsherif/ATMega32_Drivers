...
void CLCD_voidInit        ( void ){
	...
	_delay_ms(1); // wait more than 39 us
...
/*
* Brief : This Function set the curser position
* Parameters :
...
void CLCD_voidSetPosition ( u8 Copy_u8Row , u8 Copy_u8Col ){
	...
	if(Copy_u8Row>2||Copy_u8Row<1||Copy_u8Col>16||Copy_u8Col<1)  //check
	{
		...
		LOC_u8data = ( ( lcd_SetCursor ) + ( Copy_u8Col ) );              //Row1 -> 0x80+col

	}

	else if( Copy_u8Row == CLCD_ROW_2 ){

		LOC_u8data = ( ( lcd_SetCursor ) + (64) + ( Copy_u8Col ) );       //Row2 -> 0xc0+col
...
/*
* Brief : This Function send extra char
* Parameters :
            => Copy_u8Row --> row number (CLCD_ROW_1 or CLCD_ROW_2)
			=> Copy_u8Col --> column number (CLCD_COL_1 ... CLCD_COL_16)
* return : nothing
